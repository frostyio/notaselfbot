...
export const exported: Command = {
	...
	async execute(bot: Bot, message: any, args: any): Promise<MessageResult | void> {		
		...
		if (money == "all") money = Cash;
		// operations

		if (money <= 0) return new MessageError("Depositing Error", `Cannot deposit ${currency_prefix}${money}${currency_suffix}.`);

		if (money > Cash) 
			return new MessageError("Insufficient Funds", `You have insufficient funds to deposit ${currency_prefix}${money}${currency_suffix}.`);
		...
		const embed = new MessageEmbed(null, `✅ Deposited ${currency_prefix}${money}${currency_suffix}!\n\n**New Bank Balance:**`, message.author);
		embed.embed.fields = [
